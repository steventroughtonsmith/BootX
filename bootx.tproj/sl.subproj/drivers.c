...
static long FileLoadDrivers(char *dirSpec, long plugin)
{
	...
	while (1) {
		...
		length = strlen(name);
		if (strcmp(name + length - 5, ".config")) continue;
		
		sprintf(gDriverSpec, "%s\\%s", dirSpec, name);
		ret = LoadDriverPList(dirSpec, name);
		ret = LoadDriverMKext(gDriverSpec);
...
static long LoadMatchedModules(void)
{
	...
	printf("[LoadMatchedModules()] START\n");
	
	module = gModuleHead;
	while (module != 0) {
		
		printf("[LoadMatchedModules()] %i\n", module);
		...
		if (module->willLoad) {
			...
			if (length != -1) {
				...
				printf("[LoadMatchedModules()] length=%x\n", length);
