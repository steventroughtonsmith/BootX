...
long HFSInitPartition(CICell ih)
{
  ...
  if (gHFSMDB->drSigWord == kHFSSigWord) {
    gAllocationOffset = gHFSMDB->drAlBlSt * kBlockSize;
	  ...
	  //printf("[gAllocationOffset=%x]",gAllocationOffset);
	  
    // See if it is HFSPlus
    if (gHFSMDB->drEmbedSigWord != kHFSPlusSigWord) {
		//printf("[Normal HFS %x]\n", gHFSMDB->drEmbedSigWord);
      // Normal HFS;
...
//	printf("[new gAllocationOffset=%x]",gAllocationOffset);
//
//	printf("[HFS+? %x]\n",gIsHFSPlus);

  // Look for the HFSPlus Header
  Seek(ih, gAllocationOffset + kMDBBaseOffset);
  CICell c = Read(ih, (long)gHFSPlusHeader, kBlockSize);
	
//	printf("[Read? %x]\n", c);
//	printf("[gHFSPlus->signature=%x]\n", gHFSPlus->signature);
